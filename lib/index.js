...
const createFile = (currentPath, newClassName, dirStructureItem, templatesPath, stateless, needStyles) => {
    ...
    rd.on('line', function (line) {
        ...
        if (newLine.indexOf('Styles.css') != -1 && !needStyles) {
            newLine = ''
        }
        writeStream.write(`${newLine}\n`)
    })
...
const rrmw = (p) => {
    ...
    const stateless = (p.argv.indexOf('-c') != -1)
    const needStyles = (p.argv.indexOf('-s') != -1)
    ...
    dirStructure.forEach((item, i) => {
        ...
        if (!fs.existsSync(currentPath)) {
            ...
            createFile(currentPath, newClassName, item, templatesPath, stateless, needStyles)
            if (needStyles) {
